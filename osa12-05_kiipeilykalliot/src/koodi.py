...
def reittien_maaran_mukaan(kalliot: list):
    def reittien_maara(kallio: Kiipeilykallio):
        return kallio.reitteja()

    return sorted(kalliot, key=reittien_maara)


def vaikeimman_reitin_mukaan(kalliot: list):
    def vaikein(kallio: Kiipeilykallio):
        return kallio.vaikein_reitti().grade

    return sorted(kalliot, key=vaikein, reverse=True)
