...
def suorittajien_nimet(suoritukset: list):
    return list(map(lambda s: s.opiskelijan_nimi, suoritukset))


# palauttaa listan, jolla on suorituksessa olevien kurssien nimet aakkosjärjestyksessä. Kukin kurssi esiintyy listalla vain kerran.
def kurssien_nimet(suoritukset: list):
    return sorted(list(set(map(lambda s: s.kurssi, suoritukset))))
